<template>
  <div id="app">
    <page-catalogue :data-json="dataPage"></page-catalogue>
  </div>
</template>

<script>
// import dataJSON from '@static/json/kind/cellphone.json'
...
export default {
  ...
  created: function () {
    // this.dataPage = dataJSON
    // console.log(this.dataPage)
  },
  components: {PageCatalogue},
  mounted: function () {
    this.getData(this)
  },
  methods: {
    getData: function (host) {
      this.axios.get('/television/jsonData').then(function (res) {
        // Object.assign(host.dataPage, res.data) // 此语句不会触发 渲染
        // host.$forceUpdate() // 强制页面渲染 并不起作用，用this代替host, 程序报错
        // console.log(host.dataPage)
        host.dataPage = res.data
      })
    }
  }
}
