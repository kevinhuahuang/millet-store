<template>
  <div class="container">
    <div class="content">
      <div class="title">
        <segment-title com-type="配件" :link-array="linkArray" @get-link-index="getLinkIndex"></segment-title>
      </div>
...
export default {
  ...
  created: function () {
    // this.miData = miJson
    // let data = this.miData['accessories']
  ...
  methods: {
    ...
    getData: function (host) {
      this.axios.get('/miData').then(function (res) {
        ...
        let data = host.miData['accessories']
        let dataLeft = data['left']
        host.dataLeft = []
